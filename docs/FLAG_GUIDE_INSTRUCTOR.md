...
| 14 | A07 | Remember-Me Impersonation | Medium | `FLAG{A07_PREDICTABLE_REMEMBER_TOKEN_02}` in the restored-session dashboard notice | Decode `remember_lab`, change `user:N`, re-encode it, and revisit the portal. The marker is associated with understanding the cookie design; instructors may reveal it after successful impersonation. |
